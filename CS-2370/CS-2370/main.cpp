...
void outputRoster(vector<int>& jNum, vector<int>& pRate){
    int i;
    cout << "ROSTER" << endl;
    for(i=1; i < jNum.size() + 1; i++){
        cout << "Player " << i << " -- Jersey number: " << jNum.at(i-1) << ", Rating: " << pRate.at(i-1) << endl;
    }
    cout << endl;
...
void addPlayer(vector<int>& jNum, vector<int>& pRate){
    int tempNum;
    int tempRate;
    ...
    cin >> tempRate;
    jerseyNum.push_back(tempNum);
    playerRate.push_back(tempRate);
    cout << endl;
    
}
void deletePlayer(vector<int>& jNum, vector<int>& pRate){
    int input;
    ...
    if (index != -1) {
        jNum.erase(jNum.begin() + index);
        pRate.erase(pRate.begin() + index);
    } else {
        cout << "Jersey number not found." << endl;
    }
}
void updateRating(vector<int>& jNum, vector<int>& pRate){
    int input;
    cout << "Enter a jersey number:" << endl;
    cin >> input;
    ...
    cin >> newNum;
    pRate.at(index) = newNum;
    
}
void outputRating(const vector<int>& jNum, const vector<int>& pRate){
    int input;
    cout << "Enter a rating:" << endl;
    cin >> input;

    cout << endl << "ABOVE " << input << endl;
    for (int i = 0; i < pRate.size(); i++) {
        if (pRate[i] > input) {
            cout << "Player " << i+1 << " -- Jersey number: " << jNum[i] << ", Rating: " << pRate[i] << endl;
        }
...
void menu(){
    ...
    do{
        ...
        switch(input){
            case 'a': //add player
                addPlayer(jerseyNum, playerRate);
                break;
            case 'd': // remove player
                deletePlayer(jerseyNum, playerRate);
                break;
            case 'u': // update rating
                updateRating(jerseyNum, playerRate);
                break;
            case 'r': // output players above a rating
                outputRating(jerseyNum, playerRate);
                break;
            case 'o': // output roster
                outputRoster(jerseyNum, playerRate);
                break;
            case 'q': // quit
                running = false;
                break;
